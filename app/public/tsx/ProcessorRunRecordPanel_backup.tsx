...
export class ProcessorRunRecordPanel extends Component<ProcessorRunRecordPanelProps, ProcessorRunRecordPanelState> {
    ...
    processData(data:any) {
        ...
        for(let i=0; i<run_records_received.length; i++) {
            ...
            // TODO - add duration
            // Interval.fromDateTimes(date1, date2).toDuration('seconds').seconds;

              // @ts-ignore: any ...
            const run_record:ProcessorRunRecord = {
                id: entry.id,
                task: entry.task,
                start_time: DateTime.fromFormat(entry.start_time,"yyyy-MM-dd'T'H:mm:ssZ"),
                end_time: DateTime.fromFormat(entry.end_time,"yyyy-MM-dd'T'H:mm:ssZ"),
                status: entry.status
            };
    ...
    processError (data:any) {
        ...
        console.log(data)
        this.setState({error: "something went wrong: " + data.message as string})
    }
