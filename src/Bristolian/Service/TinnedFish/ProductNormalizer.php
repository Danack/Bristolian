...
/**
 * Normalizes product data from external APIs into a standard format.
 */
class ProductNormalizer
{
    ...
    public function normalizeOpenFoodFactsData(string $barcode, array $rawData): Product
    {
        ...
        [$weight, $weightDrained] = $this->parseWeight($quantity);

        // Try to extract fish species from categories or product name
        $species = $this->extractSpecies($product);
     ...
     */
    private function parseWeight(string $quantity): array
    {
        $weight = null;
        $weightDrained = null;
