...
class GetMemesSearchResponse implements StubResponse
{
    private string $body;

    /**
     * @param StoredMeme[] $memes
     */
    public function __construct(array $memes)
    {
        $converted_data = [];
        [$error, $converted_memes] = \convertToValue($memes);
        if ($error !== null) {
            throw new DataEncodingException("Could not convert memes to a value. ", $error);
        }
        $converted_data['memes'] = $converted_memes;
