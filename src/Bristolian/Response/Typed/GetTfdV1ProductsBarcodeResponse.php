...
//   Path: /api/tfd/v1/products/barcode/{barcode:.*}
//   Method: GET
// 
 ...
 * @codeCoverageIgnore
 */
class GetTfdV1ProductsBarcodeResponse implements StubResponse
{
    private string $body;
    ...
    {
        $converted_data = [];
        [$error, $converted_product] = \convertToValue($product);
        if ($error !== null) {
            throw new DataEncodingException("Could not convert product to a value. ", $error);
        }
        $converted_data['product'] = $converted_product;
