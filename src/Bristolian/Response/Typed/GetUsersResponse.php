 ...
 * @codeCoverageIgnore
 */
class GetUsersResponse implements StubResponse
{
    private string $body;

    /**
     * @param UserProfileWithDisplayName $user_info
     */
    public function __construct(UserProfileWithDisplayName $user_info)
    {
        $converted_data = [];
        [$error, $converted_user_info] = \convertToValue($user_info);
        if ($error !== null) {
            throw new DataEncodingException("Could not convert user_info to a value. ", $error);
        }
        $converted_data['user_info'] = $converted_user_info;
