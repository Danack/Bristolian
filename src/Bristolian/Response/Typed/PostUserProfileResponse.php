...
class PostUserProfileResponse implements StubResponse
{
    private string $body;

    /**
     * @param UserProfileWithDisplayName $profile
     */
    public function __construct(UserProfileWithDisplayName $profile)
    {
        $converted_data = [];
        [$error, $converted_profile] = \convertToValue($profile);
        if ($error !== null) {
            throw new DataEncodingException("Could not convert profile to a value. ", $error);
        }
        $converted_data['profile'] = $converted_profile;
