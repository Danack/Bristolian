 ...
 * @codeCoverageIgnore
 */
class GetRoomsLinksResponse implements StubResponse
{
    private string $body;
    ...
    {
        $converted_data = [];
        [$error, $converted_links] = \convertToValue($links);
        if ($error !== null) {
            throw new DataEncodingException("Could not convert links to a value. ", $error);
        }
        $converted_data['links'] = $converted_links;
