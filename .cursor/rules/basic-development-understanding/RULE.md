...
- **Exception message text in exception classes**: Define exception message text within the exception class using static constructors (e.g. `TooManyRoomTagsException::forMaxReached(int $max)`), rather than constructing the message string in application code. This keeps message formatting centralised and makes exceptions easier to reuse and test.
