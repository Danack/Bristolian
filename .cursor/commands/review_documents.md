...
When this command is called, read all documents in the `.cursor` directory and in the `notes/` directory, and note any inconsistencies or errors in them.
...
1. **Discover documents**  
   ...
   - **`notes/`** – all markdown (and other note) files, including subdirectories such as `notes/developing/`, `notes/features/`, `notes/refactoring/`, `notes/explanations/`, `notes/site/`, etc.
...
2. **Review each document**  
   ...
   - **Internal links**: Do links to other notes or anchors work? Note broken or ambiguous links.
...
3. **Compare across documents**  
   Look for:
   - **Inconsistencies**: Different instructions for the same task (e.g. how to run tests, PHPStan, or Behat) across `.cursor` vs `notes/` vs `notes/developing/`.
   ...
   - **Gaps**: Missing notes for important workflows, or notes that reference other notes that don't exist.

4. **Report**  
   Produce a concise report that:
   - Lists each document reviewed (grouped by `.cursor` vs `notes/` and subdirs).
...
- **In scope**: All document content under `.cursor/` and `notes/` (including subdirectories).
