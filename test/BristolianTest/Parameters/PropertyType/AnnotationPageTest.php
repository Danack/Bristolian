 ...
 * @coversNothing
 */
class AnnotationPageTest extends BaseTestCase
{
    /**
    ...
    /**
     * @covers \Bristolian\Parameters\PropertyType\AnnotationPage
     * @dataProvider provides_valid_input_and_expected_output
     * @param array<string, mixed> $input
     */
    public function test_parses_valid_input_to_expected_output(array $input, int $expectedValue): void
    {
        $paramTest = AnnotationPageFixture::createFromVarMap(new ArrayVarMap($input));
    ...
    {
        try {
            AnnotationPageFixture::createFromVarMap(new ArrayVarMap($input));
    ...
    {
        $propertyType = new AnnotationPage('test_name');
        $this->assertSame('test_name', $propertyType->getInputType()->getName());
    }
}

class AnnotationPageFixture implements DataType
{
    use CreateFromVarMap;
    use GetInputTypesFromAttributes;

    public function __construct(
        #[AnnotationPage('page_input')]
