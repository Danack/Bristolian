...
MASTER_PID=$(ps auwx | grep php-fpm | grep -v grep | grep 'master process'  | cut -d ' ' -f 10)
...
do
	if [[ $pid != $MASTER_PID ]]; then
		...
		nohup strace -r $summarise -s 4096 -p "$pid" $additional_strace_args >"trc/$pid.summary.trc" 2>&1 &
	fi
done < <(pgrep php-fpm)
